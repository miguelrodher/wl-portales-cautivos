...
class SupervisorController extends Controller
{
    ...
    {
        $administradores = CuentaAdministrativa::where('roles_id', '!=', 1)->get();
        return view('administradores.gestionar', compact('administradores'));
    }
    ...
    public function registroAdministradorPost(Request $request)
    {
        ...
        return redirect('/wl/supervisor/listar-administradores');
    }

    public function editarAdministrador($id_administrador)
    {
        $roles = Rol::all();
        $administrador = CuentaAdministrativa::find($id_administrador);
        return view('administradores.edit', compact('administrador', 'roles'));
    }

    public function editarAdministradorPost(Request $request, $id_administrador)
    {
        $datos_validados = $request->validate
        ([
            'nombre' => ['required', 'min:3', 'max:50', 'regex:/^[A-Za-zñÑáéíóúÁÉÍÓÚ.\s]+$/u'],
            ...
            'email' => ['required', 'email:rfc,dns', 'min:5', 'max:150', 'unique:cuentas_administrativas,email,' . $id_administrador . ',id'],
            'roles_id' => ['required'],
        ], 
        [
            // mensajes para "nombre"
        ...
        $administrador = CuentaAdministrativa::find($id_administrador);
        $administrador->nombre = $datos_validados['nombre'];
        $administrador->apellido_paterno = $datos_validados['apellido_paterno'];
        $administrador->apellido_materno = $datos_validados['apellido_materno'];
        $administrador->email = $datos_validados['email'];
        $administrador->roles_id = $datos_validados['roles_id'];

        if ($request->filled('password')) 
        {
            $administrador->password = Hash::make($datos_validados['password']);
        }
    ...
    public function cambiarEstatusAdministradorPost($id_administrador)
    {
        $administrador = CuentaAdministrativa::find($id_administrador);
    ...
    public function eliminarAdministradorPost($id_administrador)
    {
        $administrador = CuentaAdministrativa::find($id_administrador);
        $administrador->delete();

        return redirect()->route('listarAdministradores');
    }
}
