      ...
      <div class="col-sm-10 col-md-7 col-lg-5">
        <div class="card shadow-lg rounded-3">
          <div class="card-body p-4">
            ...
            {{-- Ajusta la ruta si tu ruta se llama diferente. Se asume: usuarios.update --}}
            <form method="POST" action="{{ route('editarAdministradorPost', $administrador->id) }}">
              @csrf
              ...
              <div class="mb-3">
                ...
                <input id="apellido_paterno"
                       ...
                       value="{{ old('apellido_paterno', $administrador->apellido_paterno) }}"
                       required
              ...
              <div class="mb-3">
                ...
                <input id="apellido_materno"
                       ...
                       value="{{ old('apellido_materno', $administrador->apellido_materno) }}"
                       required
              ...
              <div class="mb-3">
                ...
                <input id="email"
                       ...
                       value="{{ old('email', $administrador->correo_electronico ?? $administrador->email ?? '') }}"
                       required
              ...
              <div class="mb-3">
                ...
                <input id="password_confirmation"
                       ...
                       type="password"
                       class="form-control"
                       placeholder="Confirme su contraseña (si cambió)">
              </div>

              <div class="mb-3">
                <label class="form-label">Rol del usuario</label>
                <select name="roles_id" class="form-select @if($errors->has('roles_id')) is-invalid @endif" required>
                  <option value="">SELECCIONE UN ROL</option>
                  @foreach ($roles as $rol)
                    <option value="{{ $rol->id }}"
                      {{ (string) old('roles_id', $administrador->roles_id ?? '') === (string) $rol->id ? 'selected' : '' }}>
                      {{ $rol->rol }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="d-flex justify-content-center row mt-4 pt-3 ps-3 pe-3 ">
                <a href="{{ route('listarAdministradores') }}" class="btn btn-danger col-5 me-2">Cancelar</a>
                <button type="submit" class="btn btn-primary col-5 ms-2">Guardar cambios</button>
              </div>
            </form>
